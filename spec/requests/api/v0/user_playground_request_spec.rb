...
RSpec.describe 'UserPlaygrounds', type: :request do
  describe 'GET /user_playgrounds' do
    it 'returns all user_playgrounds' do
      ...
      get "/api/v0/users/5/favorites"

      expect(response).to have_http_status(:success)
      expect(JSON.parse(response.body).size).to eq(10)

      user_playground = JSON.parse(response.body)
      user_playground.each do |user_playground|
        expect(user_playground).to have_key('id')
        expect(user_playground['id']).to be_a(Integer)
        expect(user_playground).to have_key('user_id')
        expect(user_playground['user_id']).to be_a(Integer)
        expect(user_playground).to have_key('playground_id')
        expect(user_playground['playground_id']).to be_a(Integer)
        expect(user_playground).to have_key('playground_name')
        expect(user_playground['playground_name']).to be_a(String)
      end 
    end
  end
  ...
  describe 'POST /user_playgrounds' do
    it 'creates a new user_playground' do
      ...
      post '/api/v0/users/favorites', headers: headers, params: JSON.generate(user_playground_params)
